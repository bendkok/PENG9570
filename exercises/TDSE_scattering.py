# -*- coding: utf-8 -*-
"""
Created on Sat Jun  3 14:30:25 2023
...
def solve_while_plotting(x, psis0, Hamiltonians, times, plot_every, labels, time_propagator=Magnus_propagator, analytical=[], V=None, CAP=None, pot=1):
    ...
    for t in tqdm(range(len(times))):
        ...
        if t in [0,25,30,35,42,120,140] and CAP is not None:
            np.save(f"report/psi_CAP_{t}_{pot}", psis)
        elif t in [0,200,270,320,992]:
            np.save(f"report/psi_reg_{t}_{pot}", psis)
    ...
    if CAP is not None:
        ...
        ax1.grid()
        ax1.legend()
        title = "Velocity density function with CAP." 
        plt.title(title)
        plt.show()
...
def run_no_CAP(x0          = -30,
    ...
    for p in tqdm(range(len(p0s))):
        ...
        Hamiltonian = [make_fft_Hamiltonian(n, L, Ts[p], V=potential)[0]] # TODO: change time so it's not too long for low p0
    ...
    plt.contourf(X,Y, phi2s.T, levels=30) 
    plt.xlabel(r"$p_0$")
    plt.ylabel(r"$p$")
    ...
    if animate:
        ...
        n2 = int(n_p0/2)
        print(p0s[ 0])
        run_no_CAP_anim(x0,sigmap,p0s[ 0],tau,L,n,1000,Ts[ 0],speed,V0,w,s,d,pot_2)
        print(p0s[n2])
        run_no_CAP_anim(x0,sigmap,p0s[n2],tau,L,n,1000,Ts[n2],speed,V0,w,s,d,pot_2)
        print(p0s[-1])
        run_no_CAP_anim(x0,sigmap,p0s[-1],tau,L,n,1000,Ts[-1],speed,V0,w,s,d,pot_2)
        
    return p0s,trans_probability,refle_probability,trap_probability,k_fft,phi2s


def run_no_CAP_anim(x0          = -50,
                    sigmap      = 0.1,
                    p0          = 1.8,
                    tau         = 0,
                    L           = 700,
    ...
    res_psi = solve_while_plotting(x, psis, Hamiltonians, times, plot_every, labels, time_propagator=Magnus_propagator, V=potential, pot=pot_2)
...
def run_with_CAP(x0          = -30,
    ...
    p0s = np.linspace(p0_min, p0_max, n_p0)
    gamma_0s = np.abs(p0s) * gamma_0 # 6 / 1000
    ...
    for p in tqdm(range(len(p0s))):
        ...
        Hamiltonian = [make_fft_Hamiltonian(n,L,dt, V = potential - sp.diags(1j*CAP_vector))[0]] 
    ...
    X,Y = np.meshgrid(p0s, k_fft)
    plt.contourf(X,Y, np.abs(phi2s.T), levels=30) 
    plt.xlabel(r"$p_0$")
    plt.ylabel(r"$p$")
    plt.colorbar(label=r"$dP/dp$")
    title = "double potential" if pot_2 == 1 else "single potential"
    ...
    if animate:
        n2 = int(n_p0/2)
        run_with_CAP_anim(x0,sigmap,p0s[ 0],tau,L,n,finish_l[ 0],finish_l[ 0]*dts[ 0],int(finish_l[ 0]/200),V0,w,s,d,gamma_0s[ 0],R_part,pot_2)
        run_with_CAP_anim(x0,sigmap,p0s[n2],tau,L,n,finish_l[n2],finish_l[n2]*dts[n2],int(finish_l[n2]/200),V0,w,s,d,gamma_0s[n2],R_part,pot_2)
        run_with_CAP_anim(x0,sigmap,p0s[-1],tau,L,n,finish_l[-1],finish_l[-1]*dts[-1],int(finish_l[-1]/200),V0,w,s,d,gamma_0s[-1],R_part,pot_2)

    return p0s,Transmission,Reflection,Remainder,sums,k_fft,phi2s


def run_with_CAP_anim(x0          = -50,
                      sigmap      = 0.1,
                      p0          = 1.7,
                      tau         = 0,
                      L           = 400,
    ...
    res_psi = solve_while_plotting(x, psis, Hamiltonians, times, plot_every, labels, 
                                   time_propagator=Magnus_propagator, V=potential, CAP=[CAP_vector, exp_CAP_vector_dt, CAP_locs], pot=pot_2) 
...
def run_animations():
    ...
    run_no_CAP_anim(pot_2=0)
    run_with_CAP_anim(pot_2=0)
    run_no_CAP_anim(pot_2=1)
    run_with_CAP_anim(pot_2=1)
    

def run_p0_comparison():
    ...
    # run_with_CAP_anim(x0=-30,p0=p0_min,pot_2=1,L=300,n=512,t_steps=300,V0=3,R_part=.65,w=.5)
    # exit()
    
    # p0s,Transmission,Reflection,Remainder,sums,k_fft,phi2s
    print("\nCAP single potential: ")
    cap_sing = run_with_CAP(animate=anim, x0=x0, V0=V0, w=w, d=d, s=s, gamma_0=gamma_0, p0_min=p0_min, p0_max=p0_max, pot_2=0, n_p0=n_p0, L=250, n=512, t_steps=200, do_save=True, save_name=savename) 
    print("\nCAP double potential: ")
    cap_doub = run_with_CAP(animate=anim, x0=x0, V0=V0, w=w, d=d, s=s, gamma_0=gamma_0, p0_min=p0_min, p0_max=p0_max, pot_2=1, n_p0=n_p0, L=250, n=512, t_steps=200, do_save=True, save_name=savename) 
    
    
    print("\nNo CAP single potential: ")
    reg_sing = run_no_CAP(x0          = x0,
                          sigmap      = 0.1,
                          p0_min      = p0_min,
                          p0_max      = p0_max,
                          n_p0        = n_p0,
    ...
    simlocs = np.array([ np.where((np.abs(reg_doub[-2]-c) < 1e-6))[0] for c in cap_doub[-2]])[:,0].shape
    # cap_doub[-1] - reg_doub[-1][:,simlocs]
